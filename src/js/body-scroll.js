 ...
 * Переписано из WP плагина MouseWheel Smooth Scroll
 * Версия: 1.1.19
 * ============================================
 */
...
(function (global) {
  ...
  !(function () {
    ...
    var e = class {
      ...
      s = class {
        ...
        off(t, i) {
          this.events[t] = this.events[t]?.filter((t) => t !== i);
        }
        destroy() {
          this.events = {};
        }
        ...
        scrollTo(
          ...
          if ((!this.isStopped && !this.isLocked) || c) {
            if ("string" == typeof i && ["top", "left", "start"].includes(i))
              ...
              "string" == typeof i &&
              ["bottom", "right", "end"].includes(i)
            )
              i = this.limit;
            else {
              let t;
              if ("string" == typeof i) t = document.querySelector(i);
              else if (
                (i instanceof HTMLElement && i?.nodeType && (t = i), t)
              ) {
    ...
    // Экспорт Lenis глобально
    globalThis.Lenis = Lenis;
  })();

  // ============================================
  // Настройки LENIS (из WP плагина MouseWheel Smooth Scroll)
  // ============================================

  const lenisOptions = {
    // Автоматический requestAnimationFrame
    autoRaf: true,

    // Интенсивность линейной интерполяции (между 0 и 1)
    // меньше = более плавно, но медленнее
    lerp: 0.1,

    // Длительность анимации прокрутки в секундах
    duration: 1.2,

    // Множитель для событий колеса мыши
    wheelMultiplier: 1,
    ...
    // Горизонтальная прокрутка (по умолчанию - вертикальная)
    orientation: "vertical",

    // Плавная прокрутка колесом мыши
    smoothWheel: true,

    // Синхронизация с touch-событиями (для мобильных)
    syncTouch: false,

    // Бесконечная прокрутка
    infinite: false,

    // Разрешить overscroll (прокрутка за границы)
    overscroll: true,

    // Автоматический ресайз
    autoResize: true,

    // Поддержка якорей (anchor links)
    anchors: true,
  };
  ...
  // Глобальный доступ к экземпляру LENIS
  window.lenisInstance = lenis;
})(window);

// Экспорт для ES6 модуля
export function init() {
  // Lenis уже инициализирован
}
